...
const Register = () => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!terms) {
      pushToast(t('auth.terms_error'), 'error')
      return
    }

    if (password !== confirmPassword) {
      pushToast(t('auth.password_mismatch'), 'error')
      return
    }
    ...
    try {
      ...
      setAuth(user, token)
      pushToast(t('auth.register_success'), 'success')
      navigate('/dashboard')
    } catch (error) {
      pushToast(t('auth.register_error'), 'error')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-50 dark:bg-navy px-4 py-10">
      <div className="w-full max-w-xl rounded-[40px] border border-slate-200 dark:border-white/10 bg-white dark:bg-surface p-10 shadow-glass backdrop-blur-xl">
        <div className="mb-8 text-center">
          ...
          <h1 className="mt-4 text-4xl font-semibold text-slate-900 dark:text-white">{t('auth.create_account')}</h1>
          <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">{t('auth.register_subtitle')}</p>
        </div>

        <form className="space-y-6" onSubmit={handleSubmit}>
          <div className="space-y-2">
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">{t('auth.full_name')}</label>
            <input
              type="text"
              value={name}
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            ...
            <div className="space-y-2">
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">{t('auth.confirm_password')}</label>
              <div className="relative">
                <input
          ...
          >
            {loading ? t('auth.creating_account') : t('nav.register')}
          </button>
        </form>

        <p className="mt-8 text-center text-sm text-slate-600 dark:text-slate-400">
          {t('auth.have_account')}{' '}
          <Link to="/login" className="font-semibold text-slate-900 dark:text-white hover:text-violet">
            {t('nav.login')}
          </Link>
        </p>
      </div>
